...
export interface CreateProjectInput {
  ...
  default_branch?: { name?: string; role_name?: string; database_name?: string };
  history_retention_seconds?: number;
  org_id?: string;
}
...
export const createProject = (input: CreateProjectInput) =>
  request<CreateProjectOutput>("POST", "/projects", {
    project: {
      ...
      branch: input.default_branch ?? (input.default_branch_name
...
export const listProjects = (params: { search?: string; cursor?: string } = {}) => {
  const q = new URLSearchParams();
  if (params.search) q.set("search", params.search);
  if (params.cursor) q.set("cursor", params.cursor);
  const qs = q.toString();
  return request<ListProjectsResult>(
    "GET",
    `/projects${qs ? `?${qs}` : ""}`,
  );
};
